...
for i in range(len(names)):     #начинаем заполнение матрицы весами ребер
    ...
    for j in range(len(data)-2):
        connection = data[j+1].split(" ")   #знаем, что каждая следующая строка, после первой, содержит в себе название соединённых вершин и длину соединяющего их ребра
        for x in range(len(names)):
            if (connection[0] == names[x]):     #ищем первую вершину, указанную в строчке
                for y in range(len(names)):       #ищем вторую вершину, указанную в строчке
