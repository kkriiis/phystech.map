...
reversed_names = { i: name for i, name in enumerate(data[0].split(" "))}
len_names = len(names)

A = [[0 if i == j else math.inf     #создаем массив и заполняем его нулями по диагонали и бесконечностями не по диагонали
...
P.reverse()    #перевернули массив, содержащий оптимальный путь

with open("output_from_Kris.txt", "w") as output:
    output.write(" ".join(reversed_names[i] for i in P))